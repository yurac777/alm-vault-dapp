...
/**
 * @title  ALMVaultV11
 * @notice THE MONEY-MAKER. Delta-neutral ERC-4626 vault with performance fees.
 *         USDC collateral in Aave, short WETH, liquidity in Uniswap V3.
 */
contract ALMVaultV11 is ERC4626, Ownable {
    ...
    function setPerformanceFee(uint16 _feeBps) external onlyOwner {
        require(_feeBps <= 10000, "ALMVault: fee too high");
        performanceFeeBps = _feeBps;
    }

    function setTreasury(address _newTreasury) external onlyOwner {
        require(_newTreasury != address(0), "ALMVault: treasury cannot be zero address");
    ...
    function rebalance(uint256 uniV3ValueUSD, bytes calldata data) external onlyKeeper {
        ...
        if (currentTokenId != 0 && currentLiquidity > 0) {
            // First collect trading fees generated so far
            ...
            if (treasury != address(0) && performanceFeeBps > 0) {
                uint256 tFee0 = (fee0 * performanceFeeBps) / 10000;
                uint256 tFee1 = (fee1 * performanceFeeBps) / 10000;
                if (tFee0 > 0) IERC20(token0).safeTransfer(treasury, tFee0);
                if (tFee1 > 0) IERC20(token1).safeTransfer(treasury, tFee1);
            }
